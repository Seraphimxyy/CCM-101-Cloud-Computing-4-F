...
I'd say AWS, since it's the oldest and largest provider out of the three, and from what I found it has the widest range of services (200+), covering pretty much every use case you could think of.

Which platform would you recommend for an organization already using mostly Microsoft products?
I'd recommend Azure for that, mainly because it integrates natively with things like Windows Server, Active Directory, and Office 365, so it would probably fit right into an existing Microsoft setup without a ton of extra work.

Which platform is known for AI/ML and Kubernetes?
That would be GCP. Google actually created Kubernetes originally for their own infrastructure, and they're also known for AI/ML tools like TensorFlow and Vertex AI, so it makes sense that's their strong point.

What similarities did you observe between the three platforms?
Looking at the docs, all three offer the same core categories compute, storage, networking, and IAM  and they all use pay-as-you-go pricing along with free tiers for beginners like me to try things out.
